...
from bot import get_ans, random_chat
...
# Init object socketio
socketio = SocketIO(app)
sending_to_socket = {}

new_mess = []

# Auto-send a mess after every 5s
def auto_chat():
    global new_mess
    while True:
        # append into new_mess a sentence:
        new_mess.append(random_chat()) 
        time.sleep(5)

# An auto_bot runs auto_chat
...
# Route for short polling method
@app.route('/short')
def short():
    global new_mess
    new_mess.clear()
...
def long_chat():
    global new_mess
    while len(new_mess) < 1: # wait until length(new_mess) = 1
        pass
    ans = new_mess.pop()
